...
export default async function GetEventDashboardData(props: Props) {
  ...
  var event = await prisma.event.findFirst({ where: { event_id: props.event_id }, include: { sharedWith: true, eventLogs: { include: { user: true } }, createdBy: true, associatedWith: true, checkedIn: { include: { user: true } } } });
